...
import { countryOption, E_SOURCE, E_STAGE, E_SUBSTAGE, Role } from '@/enums';
import { optionGenerator } from '@/utils';
import { AssigneFormForm } from './AssigneForm';
...
export const AllLeadsTable: React.FC<AllLeadsTableProps> = ({ leadType }) => {
    ...
    if (leadType === 'all') {
      setQuery({
        query: {
          page: pageNo,
          limit: pageSize,
          where: { ...searchParams, [columnName]: value, stage: 'lead' },
        },
      });
    } else {
      setQuery({
            ...
            {leadType === 'all' ||
              (leadType === 'new' &&
                [Role.ADMIN, Role.MANAGER].some((role) =>
                  ...
                  <AppTableButton
                    text={`Assign ${selectedRowKeys.length} leades`}
                    onClick={() => {
                      setLeadFormTitle('Assign');
                      setOpenModal(true);
                    }}
                  />
                ))}
