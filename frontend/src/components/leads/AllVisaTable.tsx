...
import { AdminStatusChange, StatusChangeForm } from '@/components/forms';
...
export const AllVisaTable: React.FC<AllLeadsTableProps> = ({ leadType }) => {
  const [pageNo, setPageNo] = useState<number>(1);
  const [pageSize, setPageSize] = useState<number>(10);
  const [selectedRowKeys, setSelectedRowKeys] = useState<React.Key[]>([]);
  const [searchParams, setSearchParams] = useState<{ [key: string]: any }>({});
  const [openModal, setOpenModal] = useState<boolean>(false);
  const [leadFormTitle, setLeadFormTitle] = useState<string>('Change status');
          ...
          {leadType !== 'all' && (
            <ActionButton.ChangeStatus
              onClick={() => {
                ...
                setSelectedLeadApplicationCountry(record.applicationCountry);
                setOpenModal(true);
              }}
            />
          )}
    ...
    if (leadType === 'all') {
      setQuery({
        query: {
          page: pageNo,
          limit: pageSize,
          where: { ...searchParams, [columnName]: value, stage: 'compliance' },
        },
      });
    } else {
      setQuery({
        query: {
          page: pageNo,
          limit: pageSize,
          where: { ...searchParams, stage: 'compliance', subStage: leadType },
        },
      });
    }
  };
